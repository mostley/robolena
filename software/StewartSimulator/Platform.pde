class Platform {
  ...
  // REAL MEASUREMENTS
  private final float SCALE_INITIAL_HEIGHT = 205;
  private final float SCALE_BASE_RADIUS = 75;
  private final float SCALE_PLATFORM_RADIUS = 32;
  private final float SCALE_HORN_LENGTH = 30;
  private final float SCALE_LEG_LENGTH = 205;
  ...
  public float[] getDegrees(){
    float[] degrees = new float[alpha.length];

    for(int i=0; i<alpha.length; i++) {
      degrees[i] = degrees(alpha[i]);
    }
